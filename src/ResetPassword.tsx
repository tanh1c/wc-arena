...
export default function ResetPassword({ onNavigate, ...themeControls }: ResetPasswordProps) {
  ...
  async function handleResetPassword() {
    ...
    setPassword('');
    setConfirmPassword('');
    setFormNotice('Password updated. You can now sign in with your new password.');
  }
      ...
      <div className="relative z-10 min-h-screen flex flex-col p-4 lg:p-6">
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-[1fr_460px] gap-6 lg:gap-10 items-center w-full max-w-6xl mx-auto py-8 lg:py-12">
          <section className="bg-card border-4 border-main p-6 lg:p-10 shadow-[8px_8px_0_var(--color-shadow)] max-w-2xl">
            <div className="text-c2 font-black uppercase tracking-widest text-sm mb-3">Account recovery</div>
            <h1 className="text-5xl md:text-7xl font-black uppercase tracking-tighter leading-[0.88] text-main">
              Reset<br />Password
            </h1>
            <p className="font-bold text-base lg:text-lg text-subtle mt-6 max-w-xl leading-snug">
              Enter a new password after opening the secure reset link from your email.
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-3 border-2 border-main mt-8 uppercase text-xs font-black overflow-hidden">
              <div className="p-3 bg-c1 text-main sm:border-r-2 border-main flex items-center gap-2"><Trophy size={16} /> Secure</div>
              <div className="p-3 bg-c2 text-inv sm:border-r-2 border-main flex items-center gap-2"><Lock size={16} /> Private</div>
              <div className="p-3 bg-c3 text-main flex items-center gap-2"><ArrowRight size={16} /> Fast</div>
            </div>
          </section>

          <section className="bg-card border-4 border-main shadow-[8px_8px_0_var(--color-shadow)] overflow-hidden">
            <div className="bg-c2 text-inv p-4 text-center text-sm font-black uppercase">Set New Password</div>
            <div className="p-5 lg:p-6 flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label className="font-black uppercase text-xs">New password</label>
                <div className="relative">
                  <Lock size={18} className="absolute left-3 top-1/2 z-10 -translate-y-1/2 text-main opacity-80 pointer-events-none" />
                  <input type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter new password" className="w-full border-2 border-main p-3 pl-10 pr-12 font-bold text-sm bg-card shadow-[2px_2px_0_0_var(--color-shadow)] focus:shadow-[4px_4px_0_0_var(--color-shadow)] focus:ring-2 focus:ring-c2 transition-all outline-none" />
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="relative">
                  <Lock size={18} className="absolute left-3 top-1/2 z-10 -translate-y-1/2 text-main opacity-80 pointer-events-none" />
                  <input type={showConfirmPassword ? 'text' : 'password'} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Confirm new password" className="w-full border-2 border-main p-3 pl-10 pr-12 font-bold text-sm bg-card shadow-[2px_2px_0_0_var(--color-shadow)] focus:shadow-[4px_4px_0_0_var(--color-shadow)] focus:ring-2 focus:ring-c2 transition-all outline-none" />
              ...
              <button type="button" onClick={() => onNavigate('login')} className="w-full bg-card hover:bg-muted text-main font-black uppercase py-3 border-2 border-main shadow-[4px_4px_0_0_var(--color-shadow)] active:translate-y-[2px] active:translate-x-[2px] active:shadow-none transition-all text-sm flex items-center justify-center gap-3">
                Back to sign in <ArrowRight size={18} strokeWidth={3} />
              </button>
            </div>
          </section>
        </main>
