...
export default function MatchLab({ themeControls }: Props) {
  ...
  const [stoppedCheckpoint, setStoppedCheckpoint] = useState<{ id: string; hotspot: number } | null>(null);
  ...
  useEffect(() => {
    if (result?.status !== 'running' || stoppedCheckpoint?.id === result.id && stoppedCheckpoint.hotspot === result.hotspot_index) return;
    let cancelled = false;
    const token = ++requestGeneration.current;
    const previousHotspot = result.hotspot_index;
    setRunning(true);
    setError(null);
    resumeMatchLabRun(result.id, debug)
      .then((next) => {
        if (cancelled || token !== requestGeneration.current) return;
        if (next.status === 'running' && next.hotspot_index <= previousHotspot) {
          setStoppedCheckpoint({ id: next.id, hotspot: next.hotspot_index });
          setError('Match Lab stopped because it did not make progress. Retry when ready.');
        } else {
          setStoppedCheckpoint(null);
        }
        remember(next);
      })
      .catch((cause) => {
        if (cancelled || token !== requestGeneration.current) return;
        setStoppedCheckpoint({ id: result.id, hotspot: previousHotspot });
        setError(cause instanceof Error ? cause.message : 'Could not resume Match Lab.');
      })
      .finally(() => { if (!cancelled && token === requestGeneration.current) setRunning(false); });
    return () => { cancelled = true; };
  }, [debug, result?.id, result?.status, result?.hotspot_index, stoppedCheckpoint?.id, stoppedCheckpoint?.hotspot]);
  ...
  async function start() {
    ...
    setRunning(true);
    setError(null);
    try { const next = await runMatchLab(formation, botId, assignments, debug); if (token === requestGeneration.current) remember(next); }
    catch (cause) { if (token === requestGeneration.current) setError(cause instanceof Error ? cause.message : 'Match Lab failed.'); }
    finally { if (token === requestGeneration.current) setRunning(false); }
  }
  ...
  async function resume(run: MatchLabRun) {
    ...
    setRunning(true);
    setError(null);
    try { const next = await resumeMatchLabRun(run.id, debug); if (token === requestGeneration.current) remember(next); }
    catch (cause) { if (token === requestGeneration.current) setError(cause instanceof Error ? cause.message : 'Could not resume Match Lab.'); }
    finally { if (token === requestGeneration.current) setRunning(false); }
  }
  ...
  async function abandon(run: MatchLabRun) {
    ...
    setRunning(true);
    setError(null);
    try { const next = await abandonMatchLabRun(run.id); if (token === requestGeneration.current) remember({ ...run, ...next }); }
    catch (cause) { if (token === requestGeneration.current) setError(cause instanceof Error ? cause.message : 'Could not abandon Match Lab.'); }
    finally { if (token === requestGeneration.current) setRunning(false); }
  }
  ...
  async function saveFeedback() {
    ...
    try {
      ...
      if (token === requestGeneration.current) remember({ ...result, ...ratings });
    } catch (cause) { if (token === requestGeneration.current) setError(cause instanceof Error ? cause.message : 'Could not save feedback.'); }
    finally { if (token === requestGeneration.current) setRunning(false); }
  }
