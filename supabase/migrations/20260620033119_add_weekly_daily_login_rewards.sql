...
add column if not exists weekday integer check (weekday between 1 and 7);

update public.daily_login_rewards
set
  week_start_date = reward_date - (extract(isodow from reward_date)::integer - 1),
  weekday = extract(isodow from reward_date)::integer
where week_start_date is null or weekday is null;

alter table public.daily_login_rewards
alter column week_start_date set not null,
alter column weekday set not null;

drop function if exists public.claim_daily_login_reward(uuid);

create function public.claim_daily_login_reward(target_user_id uuid)
returns table (
  claimed boolean,
